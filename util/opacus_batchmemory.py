...
class BatchSplittingSampler(Sampler[List[int]]):
    ...
    def __iter__(self):
        for batch_idxs in self.sampler:
            if len(batch_idxs) == 0:
                ...
                yield []
                continue
            max_batch_size_split = int(len(batch_idxs) / self.num_splits) + 1
            # split_idxs = np.array_split(
            #     # batch_idxs, math.ceil(len(batch_idxs) / self.max_batch_size)
            # )
            split_idxs = np.array_split(
                batch_idxs, self.num_splits
            )
    ...
    def __len__(self):
        if isinstance(self.sampler, BatchSampler):
            max_batch_size_split = int(self.sampler.batch_size / self.num_splits)
            return int(
                # len(self.sampler) * (self.sampler.batch_size / self.max_batch_size)
                len(self.sampler) * self.num_splits
            )
            ...
            max_batch_size_split = int(expected_batch_size / self.num_splits)
            # return int(len(self.sampler) * (expected_batch_size / self.max_batch_size))
            return int(len(self.sampler) * self.num_splits)

        return len(self.sampler)


def wrap_data_loader(
    *, data_loader: DataLoader, num_splits: int, optimizer: DPOptimizer
):
    """
    ...
    Args:
        ...
        num_splits: number of split we want to split
...
class BatchMemoryManager(object):
    ...
    Example:
        ...
        ...     data_loader=data_loader, num_splits=2, optimizer=optimizer
        ... ) as new_data_loader:
